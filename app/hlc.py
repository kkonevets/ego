...
Python implementation of the people, but it is somewhat faster.  At least I
...
class JaccardSimilarityCalculator(object):
    ...
    def __init__(self, graph):
        self._adjlist = [
            set(graph.neighbors(i)).union([i]) for i in range(graph.vcount())
        ]
...
class TanimotoSimilarityCalculator(object):
    ...
    def __init__(self, graph, attr="weight"):
        ...
        for i in range(graph.vcount()):
            weis = dict(
                (j, weights[get_eid(i, j)]) for j in graph.neighbors(i))
            weis[i] = strengths[i] / degrees[i]
            self._adjedgelist.append(weis)

        self._sqsums = [
            sum(value * value for value in vec.values())
...
class EdgeCluster(object):
    ...
    def __repr__(self):
        return "%s(%r, %r)" % (self.__class__.__name__, self.vertices,
                               self.edges)
...
class EdgeClustering(object):
    ...
    def __init__(self, edgelist):
        ...
        parameter."""
        self.clusters = [
            EdgeCluster(edge, (i, )) for i, edge in enumerate(edgelist)
        ]
...
class HLC(object):
    ...
    def get_edge_similarity_graph(self):
        ...
        if "weight" in self.graph.edge_attributes():
            similarity = TanimotoSimilarityCalculator(
                self.graph).get_similarity_many
        elif OptimizedJaccardSimilarityCalculator.is_supported():
            similarity = OptimizedJaccardSimilarityCalculator(
                self.graph).get_similarity_many
        else:
            similarity = JaccardSimilarityCalculator(
                self.graph).get_similarity_many
    ...
    def _run_iterative(self):
        ...
        # Sort the scores
        sorted_edges = sorted(linegraph.es,
...
class HierarchicalLinkClusteringApp(object):
    ...
    def __init__(self):
        ...
        self.parser.add_option("-q",
                               ...
                               action="store_true",
                               dest="quiet",
                               help="quiet mode, print the result only")
        self.parser.add_option("-W",
                               "--no-weights",
                               action="store_true",
                               dest="no_weights",
                               help="ignore edge weights even if they "
    ...
    def process_file(self, filename):
        ...
        if graph.is_directed():
            ...
            if igraph_version < "0.6":
                ...
                self.log.warning(
                    "Mutual edges will be collapsed into a single edge.")
        ...
        if not graph.is_simple():
            self.log.warning(
                "Removing self-loops and collapsing multiple edges...")
            if igraph_version < "0.6":
